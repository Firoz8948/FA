...
export const MessageCard: React.FC<{ message: Message }> = ({ message }) => {
  ...
  return (
    <motion.div
      ...
      style={{ 
        backgroundImage: message.backgroundImage ? `url(${message.backgroundImage})` : 'none',
        backgroundSize: 'cover',
        backgroundPosition: message.backgroundPosition || 'center'
      }}
      className={cn(
        "p-8 rounded-[2rem] shadow-sm border border-pink-soft/10 flex flex-col gap-5 text-center relative overflow-hidden h-full min-h-[250px] justify-center",
        !message.backgroundImage && "bg-pink-light/30 backdrop-blur-sm"
      )}
    >
      {/* Overlay for readability if image exists */}
      {message.backgroundImage && (
        <div className="absolute inset-0 bg-white/70 backdrop-blur-[2px] transition-opacity group-hover:bg-white/60" />
      )}

      <div className="relative z-10">
        <div className="flex justify-center flex-shrink-0 mb-4 scale-110">
          {getIcon(message.emoji)}
        </div>
        <p className="text-xl md:text-2xl leading-relaxed text-pink-deep font-alice font-bold drop-shadow-sm">
          "{message.text}"
        </p>
      </div>
